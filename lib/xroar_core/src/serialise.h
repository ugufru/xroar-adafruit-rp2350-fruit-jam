...
#define ser_type_for(m) _Generic((m), \
		...
		default: _Generic((m), \
			...
			/* RP2350 port: toolchain _Generic needs an explicit default \
			 * (upstream 1.11 lacks one). The inner _Generic is type-checked \
			 * even in the unselected `default` branch, so a selector like \
			 * `unsigned int` (e.g. MC6809.state) that only matches the outer \
			 * association is ill-formed here without a default. ser_type_unhandled \
			 * is already the designed "returns control to caller" escape. */ \
